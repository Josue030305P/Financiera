 ...
 */
function populateSelect(selectElement, data, valueKey, textKey, defaultOptionText = "Seleccione una opción") {
...
async function fetchAndPopulate(url, selectElement, valueKey, textKey, defaultText) {
    try {
        ...
        showNotification('error', 'Error de carga', `No se pudieron cargar los datos necesarios: ${error.message}`);
        return [];
    }
}

async function obtenerPaises() {
    await fetchAndPopulate(`${baseUrl}app/controllers/PaisController`, paisSelect, 'idpais', 'pais', 'Seleccione un país');
}

async function obtenerPersonaToColaborador() {
    await fetchAndPopulate(`${baseUrl}app/controllers/ColaboradorController`, colaboradorSelect, 'idpersona', 'nombrecompleto', 'Seleccione una persona');
}

async function obtenerRoles() {
    await fetchAndPopulate(`${baseUrl}app/controllers/RolesController`, rolesSelect, 'idrol', 'rol', 'Seleccione un rol');
}

async function obtenerColaboradorToUsuario() {
    await fetchAndPopulate(`${baseUrl}app/controllers/UsuarioController`, usuarioSelect, 'idcolaborador', 'nombrecompleto', 'Seleccione un colaborador');
}
...
async function agregarPersonaUsuario() {
    ...
    // Validación básica
    if (!formData.idpais || !formData.apellidos || !formData.nombres || !formData.fechanacimiento || !formData.tipodocumento || !formData.numdocumento || !formData.email || !formData.telprincipal) {
        showNotification('warning', 'Campos Requeridos', 'Por favor, complete todos los campos de la Persona.');
        return;
    }
    ...
    try {
        ...
        if (resultado.status) {
            showNotification('success', '¡Éxito!', resultado.message || "Persona agregada correctamente.");
            ...
            showNotification('error', 'Error', resultado.message || "No se pudo agregar a la persona.");
        }
    } catch (error) {
        console.error("Error al agregar persona:", error);
        showNotification('error', 'Error de Conexión', "Ocurrió un error al comunicarse con el servidor al agregar persona.");
    }
}
...
async function agregarColaboradorUsuario() {
    ...
    if (!formData.idpersona || !formData.idrol || !formData.fechainicio) {
        showNotification('warning', 'Campos Requeridos', 'Por favor, complete todos los campos del Colaborador.');
        return;
    }
    ...
    try {
        ...
        if (resultado.status) {
            showNotification('success', '¡Éxito!', resultado.message || "Colaborador agregado correctamente.");
            ...
            showNotification('error', 'Error', resultado.message || "No se pudo agregar el colaborador.");
        }
    } catch (error) {
        console.error("Error al agregar colaborador:", error);
        showNotification('error', 'Error de Conexión', "Ocurrió un error al comunicarse con el servidor al agregar colaborador.");
    }
}
...
async function agregarUsuario() {
    ...
    if (!formData.get('usuario') || !formData.get('password')) { // 'usuario' en este contexto es 'nombreusuario' del input
        showNotification('warning', 'Campos Requeridos', 'Por favor, complete el nombre de usuario y la contraseña.');
        return;
    }
    if (!usuarioSelect.value) { // Validación para el select de colaborador
        showNotification('warning', 'Campos Requeridos', 'Por favor, seleccione un colaborador.');
        return;
    }
    ...
    try {
        ...
        if (resultado.status) {
            showNotification('success', '¡Éxito!', resultado.message || "Usuario agregado correctamente.");
            formUsuarioLogin.reset(); 
            fileNameDisplay.textContent = 'Ningún archivo seleccionado'; 
            showCard(cardPersona); 
           
        } else {
            showNotification('error', 'Error', resultado.message || "No se pudo agregar el usuario.");
        }
    } catch (error) {
        console.error("Error al agregar usuario:", error);
        showNotification('error', 'Error de Conexión', "Ocurrió un error al comunicarse con el servidor al agregar usuario.");
    }
}
