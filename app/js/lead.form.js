class LeadForm {
  ...
  async init() {
    await Promise.all([this.cargarAsesores(), this.cargarPaises()]);
  ...
  async cargarAsesores() {
    try {
      const response = await fetch(
        `${this.baseUrl}app/controllers/AsesorController.php`
      );
  ...
  async cargarPaises() {
    try {
      ...
      // Limpiar antes de insertar opciones
      ...
      paises.forEach((pais, index) => {
        ...
        // Si es el único país, lo marcamos como seleccionado
        if (paises.length === 1 || index === 0) {
  ...
  async cargarDatosLead() {
    try {
      const response = await fetch(
        `${this.baseUrl}app/controllers/LeadController.php?id=${this.leadId}`
      );
  ...
  async guardarLead() {
    try {
      ...
      const response = await fetch(
        `${this.baseUrl}app/controllers/LeadController.php`,
        {
          method: "POST",
          headers: {
